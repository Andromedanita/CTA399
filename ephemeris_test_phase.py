...
if __name__ == '__main__':
    ...
    mjd = Time('2013-05-16 23:47:00', scale='utc').mjd+np.linspace(0.,1.,24)
 ...
 #period for every 1000 pulse in days
    ...
    finish=time[k]+end
    while (time[k]<finish):
        f_p=eph1957.evaluate('F',mjd.tdb.mjd,t0par='PEPOCH') #frequency of pulsar
        p_p=1./(f_p[0]) #constant period of pulsar
        p_thousand=(1000*p_p) #getting period every 1000 times passes
        period=p_thousand/86400 #to make this period into days
        k+=1
...
#GMRT from tempo2-2013.3.1/T2runtime/observatory/observatories.dat
        ...
        L=(1/(1+rv))        #coefficient for finding Doppler frequency
        f_dp=(f_p[0])*L     #Doppler frequency
        p_dp=1./(f_dp)      #Doppler period
        avg=sum(p_dp)/(len(p_dp))  #average of the doppler periods
        i=4.0
        toe_res=(340-0.008*(i-120)**2)*avg  #TOA residual
        diff=abs((p_dp)-(p_p))           #difference between doppler period and period
        avg1=sum(diff)/(len(diff))       #average of this difference
        t=mjd.tdb.mjd         #arrival time of the pulsesx
        delay_day=delay/86400   #converting delays to days
        arrival=t+delay_day     #arrival times

#getting a new sample rate to fit into our period relation
        s_new=((33333333.3333)*(1.60731438719155/1000*(1-4*3.252e-07)))/avg
        time+=period
#changing format from mjd to iso
 ...
 #creating tables to display arrival times and delays
        ...
        WriteFile(arrival,"arrival_times_mjd.dat")
