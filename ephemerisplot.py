...
if __name__ == '__main__':
    ...
    mjd = Time('2013-05-16 23:47:00', scale='utc').mjd+np.linspace(0.,1.,1000)
   ...
   # while (time<finish):
    ...
    #mjd = Time('2013-05-16 23:47:00', scale='utc').mjd+np.linspace(0.,finish, steps)
    #mjd = Time(mjd, format='mjd', scale='utc', 
     #          lon=(74*u.deg+02*u.arcmin+59.07*u.arcsec).to(u.deg).value,
      #         lat=(19*u.deg+05*u.arcmin+47.46*u.arcsec).to(u.deg).value)
    #    time+=steps
...
#    arrival=t.iso
    t0 =Time('2013-05-16 23:47:00', scale='utc')
    t_0=t0.mjd
    ...
    #ab = Table([arrival, delay_day] , names=('arrival times', 'delay(days)'), meta={'name': 'first table'})
    print(arrival)
    i=0
    list1 = []
    alist = []
    while (i<1000):
        cycle=(arrival[i]-t_0)/p_p
        list1.append(cycle[i])
        phase=cycle[i]-int(cycle[i])
        alist.append(phase)
        i+=1
    plt.plot(arrival, alist)
    plt.show()
